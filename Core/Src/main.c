...
osThreadId_t ReleaseTaskHandle;
const osThreadAttr_t ReleaseTask_attributes = {
  .name = "ReleaseTask",
  .stack_size = 128 * 4,
  .priority = (osPriority_t) osPriorityLow,
};
...
void StartReleaseTask(void *argument);
...
int main(void)
{
  ...
  /* creation of ReleaseTask */
  ReleaseTaskHandle = osThreadNew(StartReleaseTask, NULL, &ReleaseTask_attributes);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(Release_GPIO_Port, Release_Pin, GPIO_PIN_RESET);
...
/* USER CODE BEGIN Header_StartReleaseTask */
/**
* @brief Function implementing the ReleaseTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartReleaseTask */
void StartReleaseTask(void *argument)
{
  /* USER CODE BEGIN StartReleaseTask */
  /* Infinite loop */
  for(;;)
  {
    osDelay(1);
  }
  /* USER CODE END StartReleaseTask */
}
